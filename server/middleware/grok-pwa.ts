...
function injectHeadStreaming(response: Response, host: string): Response {
  ...
  if (!body || typeof body.pipeThrough !== "function") return response;
...
export default async function grokPwaMiddleware(
  ...
  if (!isDocumentPath(path)) return proceed?.();

  // Returning undefined lets h3 continue to the page renderer when this file
  // is invoked without a `next` callback (handler-style, not middleware).
  if (!proceed) return;
  ...
  try {
    ...
    console.error("[grok-pwa] page renderer failed:", error);
    throw error;
  }
  ...
  if (
    ...
    try {
      ...
      console.error("[grok-pwa] head inject failed:", error);
      return result;
    }
  }
  return result;
